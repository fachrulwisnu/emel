...
import { saveEmail, getEmailUids, EmailRecord } from './db';
...
/**
 * Syncs email from POP3 server and saves new ones to SQLite.
 */
export async function fetchEmails(
  ...
  pass: string,
  limit: number = 30
): Promise<{ success: boolean; message: string; fetchedCount: number }> {
  const client = new Pop3Client();
  console.log(`\n--- [POP3 SYNC EMAILS START] ---`);
  console.log(`Target POP3 Server : ${host}:${port}`);
  console.log(`Username           : "${user}"`);

  try {
    console.log(`[POP3 Sync] Connecting to server...`);
    const greeting = await client.connect(host, port);
    console.log(`[POP3 Sync] Connected. Greeting: "${greeting.trim()}"`);
    
    // Auth - USER
    console.log(`[POP3 Sync] Authenticating user "${user}"...`);
    const userRes = await client.sendCommand(`USER ${user}`);
    console.log(`[POP3 Sync] USER Response: "${userRes.trim()}"`);
    if (!userRes.startsWith('+OK')) {
      throw new Error(`USER error: ${userRes.trim()}`);
    }

    // Auth - PASS
    console.log(`[POP3 Sync] Sending PASS command...`);
    const passRes = await client.sendCommand(`PASS ${pass}`);
    console.log(`[POP3 Sync] PASS Response: "${passRes.trim()}"`);
    if (!passRes.startsWith('+OK')) {
      throw new Error(`PASS error (Authentication failed): ${passRes.trim()}`);
    }

    // Get list of emails on server (UIDL gives mapping: <msg_num> <uid>)
    console.log(`[POP3 Sync] Sending UIDL command...`);
    const uidlRes = await client.sendCommand('UIDL', true);
    if (!uidlRes.startsWith('+OK')) {
      throw new Error(`UIDL error: ${uidlRes.trim()}`);
    }
    ...
    // Fetch existing UIDs from local DB
    const existingUids = await getEmailUids();
    const existingSet = new Set(existingUids);

    // Filter UIDs that are not in local SQLite database
    const newUids = serverUids.filter(item => !existingSet.has(item.uid));

    // Sort by message number descending (latest first)
    newUids.sort((a, b) => b.msgNum - a.msgNum);

    // Limit fetching to save bandwidth/time
    ...
    for (const item of toFetch) {
      try {
        ...
        // Un-stuff dots and separate mail header/body
        ...
        // Auto-tagging based on business rules
    ...
    return {
      success: true,
      message: `Successfully checked mailbox. Found ${newUids.length} new emails, fetched ${fetchedCount} of them.`,
      fetchedCount
    };
  } catch (err: any) {
    return {
